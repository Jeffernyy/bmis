...
if (!isset($_SESSION['role'])) {
...
} elseif (!isset($_GET['lowincome']) || !isset($_GET['resident'])) {
  header("location: ../../login.php");
  exit();

} else {
  ob_start();
  $_SESSION['clr'] = $_GET['lowincome'];
  ...
  <body>
    <div class="">
      ...
      <h1 class="text-center text-uppercase text-bold mb-5">certificate of low income</h1>
      ...
      <div class="d-flex align-items-center justify-content-center w-100 h-100">
        ...
        IF(resident_mname = 'n/a', '', CONCAT(' ', resident_mname)), ' ', resident_lname) AS lowincome_res_name
        FROM tblresident r
        LEFT JOIN tbllowincome c ON c.lowincome_res_id = r.id
        WHERE lowincome_res_id = ? AND lowincome_num = ?");

        mysqli_stmt_bind_param($btncheck_stmt, "is", $_GET['resident'], $_GET['lowincome']);
        ...
        mysqli_stmt_bind_param($tblchck_stmt, "is", $_GET['resident'], $_GET['lowincome']);
        ...
        if (mysqli_num_rows($btncheck_stmt_res) == 0) { ?>
          ...
          while ($row = mysqli_fetch_array($tblchck_stmt_res)) { ?>
            ...
            $lowincome_resident_name_class = $lowincome_resident_name === null ? 'text-danger text-lowercase' : '';

            $children_res_name = $row['lowincome_children_res_name'];
            $children_name_class = $children_res_name === null ? 'text-danger text-lowercase' : '';

            $children_res_age = $row['lowincome_children_res_age'];
            $children_age_name_class = $children_res_age === null ? 'text-danger text-lowercase' : '';
            ...
            $check_date = $row['lowincome_date_approved'] != 'n/a' || $row['lowincome_date_added'] == 'n/a' ? $row['lowincome_date_approved'] : $row['lowincome_date_added'];
            ...
            <div>
              <h4 class="mb-4" style="line-height: 1.55">&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; This is to certify that
                <strong>
                  <?php echo ucwords(strtolower(htmlspecialchars($lowincome_resident_name))) ?>,
                </strong>
                <strong>
              ...
              <h4 class="mb-4" style="line-height: 1.55">&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; This is to certify further that
                the
                aforementioned name has annual income of
                <strong>
                  <?php echo ucfirst(strtolower(htmlspecialchars($row['lowincome_annual_income']))) ?>
                </strong> with
                <?php echo numberToWords($row['lowincome_num_of_children']) ?>
                (<?php echo $row['lowincome_num_of_children'] ?>)
                children including:
                <?php
                if ($children_res_name === null) { ?>
                  <strong class="<?php echo $children_name_class ?>">
                    <?php echo $children_res_name === null ? 'children not found in the database' : ucwords(strtolower(htmlspecialchars($children_res_name))) ?>,
                  </strong>
                  <?php
                  $disablePrintButton = true;
                } else { ?>
                  <strong>
                    <?php echo ucwords(strtolower(htmlspecialchars($children_res_name))) ?>,
                  </strong>
                  <?php
                } ?>
                <?php
                if ($children_res_age === null) { ?>
                  <strong class="<?php echo $children_age_name_class ?>">
                    <?php echo $children_res_age === null ? 'age not found in the database' : htmlspecialchars($children_res_age) ?>
                  </strong>
                  <?php
                  $disablePrintButton = true;
                } else { ?>
                  <strong>
                    <?php echo htmlspecialchars($children_res_age) ?>
                  </strong>
                  <?php
                } ?> years old.
              </h4>
              ...
              <?php
              if ($row['lowincome_requester_res_id'] == 'n/a') { ?>
                <h4 class="mb-4" style="line-height: 1.55">&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; This certification is being issued
                  upon the verbal request of the above-mentioned for whatever legal purposes it may serve him/her best.
                </h4>
              <?php } elseif (is_numeric($row['lowincome_requester_res_name'])) { ?>
                <h4 class="mb-4" style="line-height: 1.55">&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; This certification is being issued
              ...
              <?php } else { ?>
                <h4 class="mb-4" style="line-height: 1.55">&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; This certification is being issued
                  ...
                  <strong class="<?php echo $requester_name_class ?>">
                    <?php echo ucwords(strtolower(htmlspecialchars($row['lowincome_requester_res_name']))) ?>,
                  </strong> in compliance with the requirement for
                  <strong>
        ...
        <div class="d-flex align-items-center justify-content-end" style="margin: 175px 0 0">
          ...
          $stmt = mysqli_prepare($con, "SELECT 
                  ...
                  FROM tbllowincome 
                  JOIN tblofficer ON tbllowincome.lowincome_officer_res_id = tblofficer.id 
                  WHERE lowincome_res_id = ? AND lowincome_num = ?");

          mysqli_stmt_bind_param($stmt, "ii", $_GET['resident'], $_GET['lowincome']);
          mysqli_stmt_execute($stmt);

          $result = mysqli_stmt_get_result($stmt);

          while ($get_officer_info = mysqli_fetch_array($result)) {
            if ($get_officer_info['lowincome_officer_res_id'] !== 'n/a' && $get_officer_info['punong_barangay_count'] > 0) { ?>
          ...
          <div style="margin: 0 40px 0 0">
            ...
            if ($count_position > 0) {
              ...
              while ($row = mysqli_fetch_array($query)) {
                ...
                ?>         <?php
                         $stmt = mysqli_prepare($con, "SELECT tbllowincome.*, tblofficer.officer_fname, tblofficer.officer_mname, tblofficer.officer_lname, tblofficer.officer_position FROM tbllowincome JOIN tblofficer ON tbllowincome.lowincome_officer_res_id = tblofficer.id WHERE lowincome_res_id = ? AND lowincome_num = ?");
                ...
                while ($get_officer_info = mysqli_fetch_array($result)) { ?>
                  <?php
                  if ($get_officer_info['lowincome_officer_res_id'] !== 'n/a') { ?>
                    <br>
                    <br>
                    <br>
                    <?php
        ...
        <button class="noprint btn btn-primary btn-block my-5" id="printpagebutton" <?php if ($disablePrintButton)
          echo 'disabled' ?> onclick="handlePrintButtonClick('#lowincome')">Print</button>
        </div>
      </body>
