                  ...
                  <div class="card card-primary card-outline">
                    <div class="card-body table-responsive">
                      <form method="post" enctype="multipart/form-data">
                        <table id="tblcaptain" class="table table-bordered table-striped table-hover">
                          <thead>
                            <tr>
                              <?php
                              if ((isset($_SESSION['role']) && $_SESSION['role'] === "administrator") || (isset($_SESSION['role']) && $_SESSION['role'] === "staff")) {
                          ...
                          <tbody>
                            ...
                            if ((isset($_SESSION['role']) && $_SESSION['role'] === "administrator")) {
                              $squery = mysqli_query($con, "SELECT *,CONCAT(captain_fname, IF(captain_mname = 'n/a', '', CONCAT(' ', captain_mname)), ' ', captain_lname) AS captain_res_name FROM tblcaptain");
                              while ($row = mysqli_fetch_array($squery)) {
                                ...
                                <tr>
                                  ...
                                  <td class="align-middle user-select-none p-2 m-0" width="50px">
                                    <image src="images/<?php echo basename($row['captain_image']) ?>" width="70px"
                                      height="70px">
                                  </td>
                                  <td class="align-middle user-select-none">
                                    <?php echo $row['captain_res_name'] ?>
                                  </td>
                                  <td class="align-middle user-select-none">
                                    <?php echo $row['captain_uname'] ?>
                                  </td>
                                  <td class="align-middle user-select-none">
                                    <?php echo $row['captain_added_by'] ?>
                                  </td>
                                  <td class="align-middle user-select-none">
                                    <?php echo $row['captain_date_added'] ?>
                                  </td>
                                  <td class="align-middle user-select-none">
...
include '../../include/footer.inc.php' ?>
  <script>
    ...
    function is_available(username, userMsgSelector, btnId) {
      ...
      $.post("../../ajax/username.ajax.php", { captain_uname: username }, function (result) {
        console.log(result);
        if (result != 0) {
